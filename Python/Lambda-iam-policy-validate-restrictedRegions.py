...
def evaluate_compliance(configuration_item, result_token):
    ...
    if resource_type == "AWS::IAM::User" or resource_type == "AWS::IAM::Role":
        ...
        for group in attached_entities["PolicyGroups"]:
            ...
            simulate_group(
                group_arn, user["GroupId"], group["GroupName"], config, iam, result_token, timestamp)
...
def simulate_principal_policy(iam, resource_arn):
    ...
    for contextValue in CONTEXT_KEY_VALUES:
        ...
        response = iam.simulate_principal_policy(
            PolicySourceArn=resource_arn, ActionNames=ACTIONS, ResourceArns=['*'], ContextEntries=[context])
...
def simulate_managed_policy(iam, policy_arn):
    ...
    for contextValue in CONTEXT_KEY_VALUES:
        ...
        context = {'ContextKeyName': CONTEXT_KEY_NAME, 'ContextKeyValues': [
            contextValue], 'ContextKeyType': 'string'}
        simulation_response = iam.simulate_custom_policy(PolicyInputList=[
                                                         policy_document], ActionNames=ACTIONS, ResourceArns=['*'], ContextEntries=[context])
...
def record_results(config, compliance_result, result_token, resource_type, resource_id, resource_arn, timestamp):
    ...
    annotation = "Entity: " + resource_arn + " is " + \
        compliance_result + " for validation of restricted EC2 region."

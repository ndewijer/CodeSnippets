...
# FROM https://github.com/Robert-W/ReInvent-2016

import json
import boto3

# Resource types this function can evaluate
APPLICABLE_RESOURCES = ["AWS::IAM::Policy"]

# Actions that we will simulate to determine compliance
POWERFUL_ACTIONS = ['iam:PutGroupPolicy', 'iam:PutRolePolicy', 'iam:PutUserPolicy', 'iam:CreatePolicy',
                    'iam:CreatePolicyVersion', 'iam:AttachGroupPolicy', 'iam:AttachRolePolicy', 'iam:AttachUserPolicy']
...
def evaluate_compliance(configuration_item, result_token):
    ...
    if resource_type == "AWS::IAM::User" or resource_type == "AWS::IAM::Role":
        ...
        record_results(config, compliance_status, result_token,
                       resource_type, resource_id, resource_arn, timestamp)
    elif resource_type == "AWS::IAM::Group":
        simulate_group(resource_arn, resource_id, resource_name,
                       config, iam, result_token, timestamp)
        ...
        attached_entities = iam.list_entities_for_policy(
            PolicyArn=resource_arn, MaxItems=1000)
        ...
        for role in attached_entities["PolicyRoles"]:
            ...
            record_results(config, compliance_status, result_token,
                           "AWS::IAM::Role", role["RoleId"], role_arn, timestamp)

        for user in attached_entities["PolicyUsers"]:
            user_arn = "arn:aws:iam:" + account_id + \
                "::user/" + user["UserName"]
            compliance_status = simulate_principal_policy(iam, user_arn)
            record_results(config, compliance_status, result_token,
                           "AWS::IAM::User", user["UserId"], user_arn, timestamp)

        for group in attached_entities["PolicyGroups"]:
            group_arn = "arn:aws:iam:" + account_id + \
                "::group/" + group["GroupName"]
            simulate_group(
                group_arn, group["GroupId"], group["GroupName"], config, iam, result_token, timestamp)


def simulate_principal_policy(iam, resource_arn):
    # Call IAM to simulate the policy on restricted actions.
    response = iam.simulate_principal_policy(
        PolicySourceArn=resource_arn, ActionNames=POWERFUL_ACTIONS, ResourceArns=['*'])
...
def simulate_managed_policy(iam, policy_arn):
    ...
    simulation_response = iam.simulate_custom_policy(
        PolicyInputList=[policy_document], ActionNames=POWERFUL_ACTIONS, ResourceArns=["*"])
...
def record_results(config, compliance_result, result_token, resource_type, resource_id, resource_arn, timestamp):
    ...
    annotation = "Entity: " + resource_arn + " is " + \
        compliance_result + " for validation of powerful actions access."
...
def simulate_group(group_arn, group_id, group_name, config, iam, result_token, timestamp):
   # Simulate group, save status
    ...
    record_results(config, compliance_status, result_token,
                   "AWS::IAM::Group", group_id, group_arn, timestamp)
    ...
    for user in users:
        ...
        record_results(config, compliance_status, result_token,
                       "AWS::IAM::User", user["UserId"], user["Arn"], timestamp)
